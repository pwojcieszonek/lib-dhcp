...
class Option39 < Minitest::Test

  # Called before every test method runs. Can be used
  # to set up fixture information.
  def setup
    @option  = Lib::DHCP::Option39.new(1)
    @option_unpack = Lib::DHCP::Option.unpack([39,1,1].pack('C3'))
  end
  ...
  def test_type
    assert_instance_of Lib::DHCP::Option39, @option
  end
  ...
  def test_oid
    assert_equal 39, @option.oid
    assert_equal 39, JSON.parse(@option.to_json)['oid']
  end

  def test_pack
    expected = [39,1,1].pack('C3')
    assert_equal expected, @option.pack
  end

  def test_type_unpack
    assert_instance_of Lib::DHCP::Option39, @option_unpack
  end
  ...
  def test_oid_unpack
    assert_equal 39, @option_unpack.oid
  end
