...
class Option41 < Minitest::Test

  # Called before every test method runs. Can be used
  # to set up fixture information.
  def setup
    @option  = Lib::DHCP::Option41.new('10.0.0.1,127.0.0.1')
    @option_unpack = Lib::DHCP::Option.unpack([41,8,167772161, 2130706433].pack('C2N2'))
  end
  ...
  def test_pack
    expected = [41,8,167772161, 2130706433].pack('C2N2')
    assert_equal expected, @option.pack
  end

  def test_type_unpack
    assert_instance_of Lib::DHCP::Option41, @option_unpack
  end
  ...
  def test_oid_unpack
    assert_equal 41, @option_unpack.oid
  end
