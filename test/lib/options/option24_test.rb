...
class Option24 < Minitest::Test

  # Called before every test method runs. Can be used
  # to set up fixture information.
  def setup
    @option  = Lib::DHCP::Option24.new(112)
    @option_unpack = Lib::DHCP::Option.unpack([24,4,112].pack('C2N'))
  end
  ...
  def test_type
    assert_instance_of Lib::DHCP::Option24, @option
  end
  ...
  def test_oid
    assert_equal 24, @option.oid
    assert_equal 24, JSON.parse(@option.to_json)['oid']
  end

  def test_pack
    expected = [24,4,112].pack('C2N')
    assert_equal expected, @option.pack
  end

  def test_type_unpack
    assert_instance_of Lib::DHCP::Option24, @option_unpack
  end
  ...
  def test_oid_unpack
    assert_equal 24, @option_unpack.oid
  end
