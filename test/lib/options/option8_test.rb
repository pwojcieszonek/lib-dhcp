...
class Option8 < Minitest::Test

  # Called before every test method runs. Can be used
  # to set up fixture information.
  def setup
    @option  = Lib::DHCP::Option8.new('10.0.0.1,127.0.0.1')
    @option_unpack = Lib::DHCP::Option.unpack([8,8,167772161, 2130706433].pack('C2N2'))
  end


  def test_type
    assert_instance_of Lib::DHCP::Option8, @option
  end
  ...
  def test_oid
    assert_equal 8, @option.oid
    assert_equal 8, JSON.parse(@option.to_json)['oid']
  end

  def test_pack
    expected = [8,8,167772161, 2130706433].pack('C2N2')
    assert_equal expected, @option.pack
  end

  def test_type_unpack
    assert_instance_of Lib::DHCP::Option8, @option_unpack
  end
  ...
  def test_oid_unpack
    assert_equal 8, @option_unpack.oid
  end
