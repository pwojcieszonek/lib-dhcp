...
class Option35 < Minitest::Test

  # Called before every test method runs. Can be used
  # to set up fixture information.
  def setup
    @option  = Lib::DHCP::Option35.new(112)
    @option_unpack = Lib::DHCP::Option.unpack([35,4,112].pack('C2N'))
  end
  ...
  def test_oid
    assert_equal 35, @option.oid
    assert_equal 35, JSON.parse(@option.to_json)['oid']
  end

  def test_pack
    expected = [35,4,112].pack('C2N')
    assert_equal expected, @option.pack
  end

  def test_type_unpack
    assert_instance_of Lib::DHCP::Option35, @option_unpack
  end
  ...
  def test_oid_unpack
    assert_equal 35, @option_unpack.oid
  end
