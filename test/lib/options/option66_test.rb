...
class Option66 < Minitest::Test

  # Called before every test method runs. Can be used
  # to set up fixture information.
  def setup
    @option  = Lib::DHCP::Option66.new('test')
    @option_unpack = Lib::DHCP::Option.unpack([66,4,'test'].pack('C2a4'))
  end
  ...
  def test_type
    assert_instance_of Lib::DHCP::Option66, @option
  end
  ...
  def test_oid
    assert_equal 66, @option.oid
    assert_equal 66, JSON.parse(@option.to_json)['oid']
  end

  def test_pack
    expected = [66,4,'test'].pack('C2a4')
    assert_equal expected, @option.pack
  end

  def test_type_unpack
    assert_instance_of Lib::DHCP::Option66, @option_unpack
  end
  ...
  def test_oid_unpack
    assert_equal 66, @option_unpack.oid
  end
