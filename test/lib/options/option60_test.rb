...
class Option60 < Minitest::Test

  # Called before every test method runs. Can be used
  # to set up fixture information.
  def setup
    @option  = Lib::DHCP::Option60.new('test')
    @option_unpack = Lib::DHCP::Option.unpack([60,4,'test'].pack('C2a4'))
  end
  ...
  def test_type
    assert_instance_of Lib::DHCP::Option60, @option
  end
  ...
  def test_oid
    assert_equal 60, @option.oid
    assert_equal 60, JSON.parse(@option.to_json)['oid']
  end

  def test_pack
    expected = [60,4,'test'].pack('C2a4')
    assert_equal expected, @option.pack
  end

  def test_type_unpack
    assert_instance_of Lib::DHCP::Option60, @option_unpack
  end
  ...
  def test_oid_unpack
    assert_equal 60, @option_unpack.oid
  end
